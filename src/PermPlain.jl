...
export permcycles, cyclelengths, permsgn, permorder,
       ...
       permtotrans, cycletype, ispermcycs, permlistisequal,
       canoncycles, cycstoperm, cycleprint, permarrprint, cycpow,
...
# return the order of the permutation from PCYC
permordercyc(cyc) = permorder_from_lengths([length(c) for c in cyc])
...
function permpower{T<:Real}(p::AbstractVector{T}, n::Int)
...
# Compute power of permutation. Input is PCYC
# Output is PLIST
# see pari perm.c
function cycpow(cyc, exp)
    r = 1
